...
| `stg_quickbooks__exchange_rate`, `stg_quickbooks__exchange_rate_tmp` | New modelsf | |  | Brings in the new `EXCHANGE_RATE` source table so historical exchange rates can be looked up by date and currency pair. Consumed by `int_quickbooks__general_ledger_balances` to look up the report-date rate for the opt-in conversion described below. |
