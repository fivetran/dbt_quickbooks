...
## Under the Hood
- Introduces the `partition_by_source_relation` macro to conditionally include `source_relation` in window function PARTITION BY clauses only when multiple sources are configured via the `quickbooks_union_schemas` and `quickbooks_union_databases` variables. 
