...
final as (
    ...
    select
        *,
        sum(adjusted_amount) over (partition by account_id, class_id {{ fivetran_utils.partition_by_source_relation(package_name='quickbooks') }}
            order by transaction_date, transaction_id, transaction_index rows unbounded preceding) as running_balance,
        sum(adjusted_converted_amount) over (partition by account_id, class_id {{ fivetran_utils.partition_by_source_relation(package_name='quickbooks') }}
            order by transaction_date, transaction_id, transaction_index rows unbounded preceding) as running_converted_balance
    from adjusted_gl
)
