...
| [quickbooks__inventory_item_activity](https://fivetran.github.io/dbt_quickbooks/#!/model/model.quickbooks.quickbooks__inventory_item_activity) | Line-level view of all item-bearing transactions across purchases, bills, vendor credits, invoices, sales receipts, credit memos, refund receipts, and (optionally) purchase orders. Each row includes item enrichment details, quantity, unit price, and a `transaction_direction` indicator (inbound/outbound/pending) for inventory movement analysis. <br><br>**Example Analytics Questions:**<br><ul><li>Which items move the most volume across transaction types, and how does that vary by period?</li><li>What is the net item movement by SKU across inbound and outbound transactions?</li></ul> |
