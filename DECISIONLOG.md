...
- In the `int_quickbooks__cash_flow_classifications` model, we classify the `cash_flow_type` based on the `account_type`, `account_class` or `account_name` fields of the balance sheet line. This is based on financial best practices. Cash flow types usually fall into one of four buckets:
