...
## Having a Single Accounts Payable Account
The `int_quickbooks__bill_payment_double_entry` model requires a single account designated as 'Accounts Payable' in the `account_type` field. This aligns with QuickBooks Online requirements and accounting best practices. If you have multiple accounts designated as 'Accounts Payable', the [join logic](https://github.com/fivetran/dbt_quickbooks/blob/main/models/double_entry_transactions/int_quickbooks__bill_payment_double_entry.sql#L57) brings all Accounts Payable accounts into the join, causing data fanout. Variable configurability to select a specific Accounts Payable account is planned for future releases, however having a single A/P account is in keeping with Quickbooks' recommendation and accounting best practices.